//! The example shows how to use a context with Lake Framework.
//! It is going to follow the NEAR Social contract and the block height along
//! with a number of calls to the contract.
use near_lake_framework::{LakeContext, near_lake_primitives};
use std::io::Write;
...
// That trait requires implementing two methods `execute_before_run` and `execute_after_run`.
// However, we don't actually need them in our cause of using the context.
// That's why we're using the derived macro to implement the trait for us.
